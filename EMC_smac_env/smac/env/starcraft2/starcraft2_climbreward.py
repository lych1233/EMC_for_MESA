...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        new_suboptimal_reward_scale=0.5,
        new_getdamage_penalty_scale=0,
        new_risk_penalty_scale=1,
        new_defeat_bonus=0,
        target_enemy=0,
        K_agents=3, 
        map_name="8m",
        ...
        """
        # Climb_game-like arguements
        self.original_battle_reward_factor = original_battle_reward_factor
        self.new_suboptimal_reward_scale = new_suboptimal_reward_scale
        self.new_getdamage_penalty_scale = new_getdamage_penalty_scale
        self.new_risk_penalty_scale = new_risk_penalty_scale
        self.new_defeat_bonus = new_defeat_bonus
        self.target_enemy = target_enemy
        self.K_agents = K_agents
        if new_getdamage_penalty_scale is not None:
            reward_negative_scale = new_getdamage_penalty_scale
        if new_defeat_bonus is not None:
            reward_defeat = new_defeat_bonus
        ...
        # )
        self.max_reward = self.n_enemies * (self.new_defeat_bonus + 150)
    ...
    def step(self, actions):
        ...
        terminated = False
        reward_climb_factor = self.reward_climb_factor(actions_int)
        _reward_battle = self.reward_battle()
        reward = (reward_climb_factor + self.original_battle_reward_factor) * _reward_battle
        info = {"battle_won": False, "total_damage": self._total_caused_damage}
        info.update(self.hit_info)
    ...
    def reward_climb_factor(self, actions):
        """
        Now we will add one another scaling factor:
            1 for K attacks on target_enemy
            0.5 for 0 attacks on target_enemy
            0 for [1, K-1] attacks on target_enemy
            Above rewards are given only if all the agents attacks some enemies
        """
        reward = 0
        ...
        for action in actions:
            ...
            if target_id >= 0:
                n_attacks += 1
                if target_id == self.target_enemy:
                    attack_to_target += 1
        assert n_alives == n_non_noop
        if n_attacks == n_alives:        
            self.hit_info["all_attack"] += 1
            if attack_to_target == self.K_agents:
                self.hit_info["optimal_attack"] += 1
                reward = 1
            elif attack_to_target == 0:
                self.hit_info["suboptimal_attack"] += 1
                reward = self.new_suboptimal_reward_scale
            else:
                self.hit_info["failed_attack"] += 1
                reward = self.new_suboptimal_reward_scale * (1 - self.new_risk_penalty_scale)
        return reward
    ...
    def reward_battle(self):
        ...
        self._total_caused_damage += delta_enemy
