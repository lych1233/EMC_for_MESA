...
from modules.mixers.maven_qmix import QMixer
...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.double_q:
            ...
            new_mac_out = mac_out.detach().clone()
            new_mac_out[avail_actions == 0] = -9999999
            cur_max_actions = new_mac_out[:, 1:].max(dim=3, keepdim=True)[1]
        ...
        # Discriminator
        new_mac_out[avail_actions == 0] = -9999999
        q_softmax_actions = th.nn.functional.softmax(new_mac_out[:, :-1], dim=3)
