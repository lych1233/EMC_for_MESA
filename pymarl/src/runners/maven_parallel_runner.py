...
from components.maven_episode_buffer import EpisodeBatch
...
class ParallelRunner:
    ...
    def run(self, test_mode=False, test_uniform=False, acquire_reward=False):
        ...
        while True:
            ...
            step_rewards = np.zeros(self.batch_size)
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    data = parent_conn.recv()
                    # Remaining data for this current timestep
                    post_transition_data["reward"].append((data["reward"],))
                    step_rewards[idx] = data["reward"]
        ...
        if acquire_reward:
            return self.batch, np.stack(all_rewards)
        return self.batch
