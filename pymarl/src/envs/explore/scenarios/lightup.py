...
class Scenario(BaseScenario):
    def make_world(self, args, seed=None):
        ...
        world.landmarks[world.args.target_id].color = np.array([0.25, 0.25, 0.75])
    ...
    def reward(self, agent, world):
        ...
        if self._light_up(goal):
            cnt_in_target = self._count_agent_in_landmark(goal, world)
            rew = 1. if cnt_in_target == world.num_agents else 0.
        else:
            for i in range(world.num_landmarks):
                landmark = world.landmarks[i]
                if landmark is target:
                    continue
                if self._count_agent_in_landmark(landmark, world) == world.num_agents:
                    rew = 0.05
                    break
